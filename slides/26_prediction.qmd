...
```{r, message = FALSE, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 5, fig.alt="Scatter plots comparing Training Data and Test Data."}
library(ggpubr)
# generate data
set.seed(302)
n <- 30
...
```{r, message = FALSE, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 5, fig.alt="Scatter plots showing training data with an error of 3.309 and test data with an error of 3.866, both with a positive trend line."}
# Fit linear model and calculate training error
lm_fit <- lm(y ~ x, data = df_train)
yhat_train <- predict(lm_fit)
# (y_i - yhat_i)^2
...
```{r, message = FALSE, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 5, fig.alt = "Side-by-side scatter plots comparing training data (left) and test data (right) with trend lines."}
# Fit 5 degree polynomial and calculate training error
lm_fit_5 <- lm(y ~ poly(x, 5), data = df_train)
yhat_train_5 <- predict(lm_fit_5)
train_err_5 <- mean((df_train$y - yhat_train_5)^2)
...
```{r, message = FALSE, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 5, fig.alt = "Two scatter plots showing training data and test data with fitted red curves and respective errors: Training error is 2.264 and Test error is 8.193."}
# Fit 10 degree polynomial and calculate training error
lm_fit_10 <- lm(y ~ poly(x, 10), data = df_train)
yhat_train_10 <- predict(lm_fit_10)
train_err_10 <- mean((df_train$y - yhat_train_10)^2)
...
```{r, message = FALSE, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 5, fig.alt = "Two scatter plots titled Training Data and Test Data with respective training and test errors. Both have black data points and a red curve fitting the points."}
# Fit 15 degree polynomial and calculate training error
lm_fit_15 <- lm(y ~ poly(x, 15), data = df_train)
yhat_train_15 <- predict(lm_fit_15)
train_err_15 <- mean((df_train$y - yhat_train_15)^2)
...
Four target charts illustrating combinations of bias and variance: "low bias, low variance," "low bias, high variance," "high bias, low variance," and "high bias, high variance."
...
Three graphs showing underfitted, well-fitted, and overfitted data models.
...
Three scatterplots depicting under-fitting, appropriate-fitting, and over-fitting in data fitting, with blue lines representing different fit types.
...
```{r, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 6, fig.alt = "Scatter plot titled Sample-splitting, showing red dots for Training and cyan dots for Test samples."}
library(forcats)
# Generate data
set.seed(302)
n <- 50
...
```{r, echo = TRUE, eval = TRUE, fig.align = "center", fig.height = 6, fig.alt="Comparison scatter plots with training and test data; left shows a linear fit with degree 1, right shows a complex model with degree 10."}
...
Data split into training (blue) and test (red) sets arranged in rows and columns.
...
```{r, echo = FALSE, eval = TRUE, fig.align = "center", fig.height = 6, fig.alt = "Two scatter plots showing cross-validation results with a linear fit on the left (Degree 1 CV error: 4.24) and a complex fit on the right (Degree 10 CV error: 17.372)."}
# Create predictive model
lm_1 <- lm(y ~ x, data = data)
lm_10 <- lm(y ~ poly(x, 10), data = data)
...
![](images/knn.png){fig-align="center" fig-alt="Side-by-side scatter plots; the left has an "X" within a green circle amidst scattered blue and orange dots, and the right shows a grid of tiny blue and orange dots divided by irregular lines."}

Image Credit: An Introduction to Statistical Learning by Gareth James, Daniela Witten, Trevor Hastie, Robert Tibshirani

## k-nearest neighbors

![](images/knn2.png){fig-align="center" fig-alt="KNN classification comparison with K=1 and K=100, showing different boundary shapes between two classes of data points."}
...
![](images/choosek.png){fig-alt="Line graph showing training and test error rates versus ( \\frac{1}{K} ). Training errors (turquoise) decrease to zero, test errors (orange) fluctuate and increase around 0.1. Legend included." fig-align="center"}

## Choosing k

![](images/doubledescent.png){fig-alt="Graph showing training error (blue squares) decreasing and test error (black circles) initially decreasing, sharply increasing, and then decreasing with annotation indicating \"Interpolation Threshold.\"" fig-align="center"}
...
![](images/tree.png){fig-alt="Scatter plot titled \"Tree with 5 leaves,\" divided into five colored regions with black data points." fig-align="center"}
...
![](images/randomforest.png){fig-alt="Diagram showing a dataset leading into three decision trees, with their mean providing a prediction." fig-align="center"}

[Image source](https://towardsdatascience.com/random-forests-and-decision-trees-from-scratch-in-python-3e4fa5ae4249)

## Random Forest

![](images/randomforest2.jpeg){fig-alt="A 3x3 grid of hierarchical tree diagrams predicting either 1 or 0, with a tally indicating six 1s and three 0s. Overall prediction is 1." fig-align="center"}
